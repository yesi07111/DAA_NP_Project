...
Un problema es APX-Complete si es APX-Hard y, además, pertenece a la clase APX (admite un algoritmo de aproximación de factor constante $c$). Ya establecimos que el MCCPP es APX-Hard (Sección 2.5.1). Ahora demostraremos que \textbf{no pertenece a APX}, lo cual tiene consecuencias fundamentales sobre su aproximabilidad.

\paragraph{Teorema (MCCPP no pertenece a APX):} El Problema de Partición Cromática de Costo Mínimo no admite un algoritmo de aproximación de factor constante $c$ en tiempo polinomial, a menos que P = NP. Por tanto, MCCPP $\notin$ APX.

\paragraph{Demostración:}

La demostración se realiza mediante una reducción desde el problema de Maximum Weight Independent Set (MWIS), cuya inaproximabilidad está bien establecida en la literatura.

\textbf{Paso 1: Conexión entre MCCPP y Maximum Weight Independent Set}

Consideremos una instancia del MCCPP con la siguiente estructura particular:
\begin{itemize}
	\item Grafo $G = (V, E)$ arbitrario
	\item Conjunto de frecuencias $F = \{f_0, f_1, \ldots, f_n\}$ donde $|F| = n + 1$
	\item Matriz de costos definida como:
	\begin{equation}
		c_{v,f} = \begin{cases}
			0 & \text{si } f = f_0 \\
			M & \text{si } f \neq f_0
		\end{cases}
	\end{equation}
	donde $M$ es una constante arbitrariamente grande (por ejemplo, $M = n \cdot \max_{v,f'} c_{v,f'} + 1$).
\end{itemize}

En esta instancia, cualquier solución óptima del MCCPP debe:
\begin{enumerate}
	\item Asignar el color $f_0$ (costo cero) al máximo número posible de vértices.
	\item Los vértices restantes recibirán algún color $f_i \neq f_0$ con costo $M$.
\end{enumerate}

Sea $I^*$ el conjunto de vértices que reciben el color $f_0$ en la solución óptima. Por la restricción de coloración propia:
\begin{equation}
	\forall u, v \in I^*: \{u, v\} \notin E
\end{equation}

Es decir, $I^*$ es un conjunto independiente. El costo óptimo de esta instancia es:
\begin{equation}
	Z^* = 0 \cdot |I^*| + M \cdot (n - |I^*|) = M(n - |I^*|)
\end{equation}

Minimizar $Z^*$ es equivalente a \textbf{maximizar} $|I^*|$, que es precisamente el problema de Maximum Independent Set.

\textbf{Paso 2: Extensión a Maximum Weight Independent Set}

Generalizamos la construcción anterior asignando pesos a los vértices mediante la matriz de costos:
\begin{equation}
	c_{v,f} = \begin{cases}
		M - w(v) & \text{si } f = f_0 \\
		M & \text{si } f \neq f_0
...
donde $w(v) > 0$ es el peso del vértice $v$ en la instancia de MWIS. Ahora el costo óptimo es:
\begin{equation}
	Z^* = \sum_{v \in I^*}(M - w(v)) + M \cdot (n - |I^*|) = Mn - \sum_{v \in I^*} w(v)
\end{equation}

Minimizar $Z^*$ es equivalente a maximizar $\sum_{v \in I^*} w(v)$, que es MWIS.

\textbf{Paso 3: Resultados de Inaproximabilidad de MWIS}

Maximum Weight Independent Set es uno de los problemas canónicos de inaproximabilidad. Los resultados fundamentales establecen:

\begin{itemize}
	\item \textbf{Håstad (1999):} Independent Set no puede aproximarse en factor $n^{1-\epsilon}$ para cualquier $\epsilon > 0$, a menos que P = NP \cite{hastad1999clique}.
	
	\item \textbf{Zuckerman (2007):} Bajo la conjetura de que NP $\nsubseteq$ BPTIME($2^{n^{O(1)}}$), Independent Set no puede aproximarse en factor $n / (\log n)^c$ para cualquier constante $c$ \cite{zuckerman2007linear}.
\end{itemize}

Estos resultados se extienden directamente a la versión ponderada (MWIS) mediante reducciones estándar.

\textbf{Paso 4: Conclusión por Contradicción}

Supongamos, por contradicción, que existe un algoritmo de aproximación de factor constante $\alpha \geq 1$ para el MCCPP que produce una solución $Z_{Alg}$ tal que:
\begin{equation}
	Z_{Alg} \leq \alpha \cdot Z^*
\end{equation}

Para la instancia construida en el Paso 2:
\begin{align}
	Z_{Alg} &= Mn - \sum_{v \in I_{Alg}} w(v) \\
	Z^* &= Mn - \sum_{v \in I^*} w(v)
\end{align}

donde $I_{Alg}$ es el conjunto independiente implícito en la solución del algoritmo e $I^*$ es el conjunto independiente óptimo.

De la condición de aproximación:
\begin{align}
	Mn - \sum_{v \in I_{Alg}} w(v) &\leq \alpha \left(Mn - \sum_{v \in I^*} w(v)\right) \\
	Mn - \sum_{v \in I_{Alg}} w(v) &\leq \alpha Mn - \alpha \sum_{v \in I^*} w(v) \\
	\alpha \sum_{v \in I^*} w(v) - \sum_{v \in I_{Alg}} w(v) &\leq (\alpha - 1)Mn
\end{align}

Para $M$ suficientemente grande (específicamente, $M > \frac{\alpha \cdot W_{max}}{\alpha - 1}$ donde $W_{max} = \max_v w(v) \cdot n$), el término $(\alpha - 1)Mn$ domina, y podemos reorganizar:
\begin{equation}
	\sum_{v \in I_{Alg}} w(v) \geq \frac{1}{\alpha} \sum_{v \in I^*} w(v) - O(1)
\end{equation}

Para instancias donde $\sum_{v \in I^*} w(v) \gg 1$, esto implica que $I_{Alg}$ es una $\alpha$-aproximación para MWIS.

Pero esto contradice los resultados de inaproximabilidad de MWIS, que establecen que no existe aproximación de factor constante para MWIS a menos que P = NP.

Por tanto, no puede existir un algoritmo de aproximación de factor constante para el MCCPP. $\square$

\paragraph{Corolario (Clasificación en la Jerarquía de Aproximabilidad):}

\begin{enumerate}
	\item \textbf{MCCPP $\notin$ APX:} Como no admite aproximación de factor constante, el MCCPP no pertenece a la clase APX.
	
	\item \textbf{MCCPP $\in$ log-APX:} Aunque no pertenece a APX, existen algoritmos de aproximación con factor logarítmico. Específicamente, mediante técnicas de Set Cover y relajación primal-dual, se pueden obtener algoritmos con garantía $O(\ln |V|)$ \cite{feige1998threshold}.
	
	\item \textbf{No es APX-Complete:} Para ser APX-Complete, un problema debe estar en APX. Como MCCPP $\notin$ APX, no puede ser APX-Complete, aunque sí es APX-Hard.
\end{enumerate}

\paragraph{Implicaciones Prácticas:}

La clasificación del MCCPP en log-APX (pero fuera de APX) tiene consecuencias importantes para el diseño de algoritmos:

\begin{itemize}
	\item \textbf{Imposibilidad de PTAS:} No existe un esquema de aproximación en tiempo polinomial que alcance $(1+\epsilon)$-aproximación para cualquier $\epsilon > 0$.
	
	\item \textbf{Mejor garantía alcanzable:} Los algoritmos deben conformarse con factores logarítmicos $O(\ln n)$ o metaheurísticas sin garantías teóricas.
	
	\item \textbf{Enfoque pragmático:} Para instancias realistas, las metaheurísticas (Simulated Annealing, búsqueda tabú, algoritmos genéticos) son la opción más viable, ya que los algoritmos con garantía logarítmica pueden tener constantes multiplicativas prohibitivamente grandes.
\end{itemize}
...
\begin{itemize}
	...
	\item \textbf{Complejidad total:} $k^n \cdot (m + n) = k^n \cdot O(m + n) = \Theta(k^n \cdot (m + n))$
\end{itemize}
...
\begin{itemize}
	...
	\item Por tanto: $\Omega(k^n \cdot (m + n))$.
\end{itemize}
...
\textbf{T (Tiempo de Ejecución):} Para cada uno de los $n$ vértices y $k$ colores, sumamos sobre los hijos. La complejidad es $O(n \cdot k^2)$. Si además mantenemos el mínimo y el segundo mínimo de los costos de los hijos, se puede reducir a $O(n \cdot k)$.
...
\begin{itemize}
	...
	\item Existe una optimización estándar (mantener para cada hijo el mínimo y segundo mínimo y el correspondiente color), que reduce cada consulta a \(O(1)\) y lleva la complejidad a:
...
\textbf{Algoritmo DP para Grafos de Intervalo mediante esquema SRTBOT:\\}
...
\textbf{S (Subproblemas):} Dada una representación de intervalos válida $I(v) = [l(v), r(v)]$ para cada vértice $v$, ordenamos los vértices por el extremo derecho de sus intervalos, obteniendo la secuencia $v_1, v_2, \dots, v_n$ donde $r(v_1) \leq r(v_2) \leq \dots \leq r(v_n)$.

El estado de programación dinámica se define como $DP[i][\sigma]$, que representa el costo mínimo de colorear los primeros $i$ vértices en el ordenamiento, dado que el estado $\sigma$ especifica la coloración de los intervalos que permanecen activos en la posición $i$. Formalmente, un intervalo $v_j$ (con $j < i$) está activo en la posición $i$ si $r(v_j) \geq l(v_i)$, es decir, si su intervalo se solapa con el intervalo del vértice que se está procesando.
...
\textbf{R (Recurrencia):} Para $i > 1$, la recurrencia se define mediante el proceso de extensión desde estados anteriores. Para cada estado previo $DP[i-1][\sigma']$, construimos el conjunto de intervalos activos filtrando aquellos que ya han terminado. Específicamente, un intervalo $v_j$ incluido en $\sigma'$ permanece activo si $r(v_j) \geq l(v_i)$.

Sea $\sigma'_{\text{activo}}$ el estado resultante tras eliminar los intervalos inactivos de $\sigma'$. Para cada color $c \in F$ disponible para el vértice $v_i$, verificamos la compatibilidad con los intervalos activos. La condición de compatibilidad requiere que para todo $(v_j, c') \in \sigma'_{\text{activo}}$ tal que $v_j$ es adyacente a $v_i$, se cumpla que $c \neq c'$.

Si la compatibilidad se satisface, el nuevo estado $\sigma$ se construye como $\sigma = \sigma'_{\text{activo}} \cup \{(v_i, c)\}$, y el costo asociado se calcula como:
...
\textbf{T (Topología):} El procesamiento sigue un orden lineal ascendente según el ordenamiento de los vértices por el extremo derecho de sus intervalos. Este ordenamiento garantiza que cuando procesamos el vértice $v_i$, todos los vértices $v_j$ con $j < i$ ya han sido considerados, y podemos determinar precisamente cuáles de ellos permanecen activos basándonos en la comparación de sus extremos derechos con el extremo izquierdo de $v_i$.

\textbf{B (Base):} Para el primer vértice $v_1$ en el ordenamiento, no existen vértices previamente procesados, por lo que el conjunto de intervalos activos está vacío. Para cada color $c \in F$, establecemos:
...
\textbf{O (Objetivo):} La solución óptima se obtiene identificando el estado con costo mínimo en la última etapa del procesamiento:

\[
Z^* = \min\{DP[n][\sigma] : \sigma \text{ es un estado válido en la posición } n\}
\]

Cada estado $\sigma$ en $DP[n]$ representa una coloración completa y propia del grafo, ya que todos los $n$ vértices han sido procesados. La coloración específica asociada al estado óptimo se recupera mediante el seguimiento de las decisiones tomadas durante la construcción de la tabla de programación dinámica.

\textbf{T (Tiempo):} La complejidad temporal del algoritmo está determinada por tres factores principales. El preprocesamiento, que incluye la verificación de cordalidad, la construcción de la representación de intervalos y el ordenamiento de vértices, requiere $O(n^2 + m)$ operaciones en el peor caso. El cálculo de la tabla de programación dinámica procesa $n$ etapas, una por cada vértice. En cada etapa $i$, el número de estados posibles está acotado por $O(k^{\omega_i})$ donde $\omega_i$ es el número máximo de intervalos que se solapan simultáneamente hasta la posición $i$. Para cada estado, se evalúan $k$ posibles colores para el vértice actual, y la verificación de compatibilidad con los intervalos activos requiere $O(\omega_i)$ operaciones.

En grafos de intervalo típicos, el valor $\omega$ (tamaño de la clique máxima) es mucho menor que $n$, lo que hace el algoritmo eficiente en la práctica. La complejidad total resulta en $O(n^2 + n \cdot k^{\omega+1} \cdot \omega)$, que se simplifica a $O(n \cdot k^{\omega+1} \cdot \omega)$ cuando el grafo es suficientemente denso. Para grafos dispersos con cliques pequeñas, esta complejidad es polinomial en la práctica.
...
\begin{itemize}
	\item \textbf{Preprocesamiento:} La fase de preprocesamiento consta de varias operaciones secuenciales. La verificación de cordalidad mediante Maximum Cardinality Search se ejecuta en tiempo $O(n+m)$. La construcción de la representación de intervalos, que involucra el análisis de las relaciones de vecindad para cada vértice y la asignación de coordenadas de intervalos, requiere $O(n^2)$ operaciones en el peor caso. La validación de que la representación construida es consistente con el grafo original implica verificar para cada par de vértices si la condición de solapamiento de intervalos corresponde a la existencia de una arista, lo cual también requiere $O(n^2)$ comparaciones. Finalmente, el ordenamiento de los vértices por el extremo derecho de sus intervalos se realiza en $O(n \log n)$. El preprocesamiento total es por tanto $O(n^2)$ en el peor caso.
	
	\item \textbf{Cálculo DP:} El cálculo de la tabla procede en $n$ etapas, una por cada vértice en el ordenamiento. En la etapa $i$, el algoritmo debe considerar todos los estados válidos de la etapa anterior $DP[i-1]$. El número de estados en $DP[i-1]$ está acotado superiormente por el número de coloraciones distintas posibles de los intervalos que están activos en esa posición. Si hay $\omega_{i-1}$ intervalos activos en la posición $i-1$, existen potencialmente $k^{\omega_{i-1}}$ estados distintos, aunque en la práctica este número es menor debido a restricciones de compatibilidad.
	
	Para cada estado $\sigma'$ en $DP[i-1]$, el algoritmo realiza las siguientes operaciones. Primero, filtra los intervalos activos eliminando aquellos cuyo extremo derecho es menor que el extremo izquierdo del vértice actual, lo cual requiere $O(\omega_{i-1})$ comparaciones. Segundo, para cada uno de los $k$ colores posibles del vértice $v_i$, verifica la compatibilidad con los intervalos activos revisando si algún vecino activo tiene el mismo color, operación que requiere $O(\omega_i)$ verificaciones de adyacencia. Tercero, si la asignación es compatible, construye el nuevo estado y actualiza la tabla $DP[i]$.
	
	El número de estados en cada etapa $i$ está acotado por $k^{\omega_i}$, y cada transición desde un estado previo a un nuevo estado requiere $O(k \cdot \omega_i)$ operaciones. Sumando sobre todas las etapas, obtenemos una complejidad de $O(n \cdot k^\omega \cdot k \cdot \omega) = O(n \cdot k^{\omega+1} \cdot \omega)$ para el cálculo completo de la tabla de programación dinámica.
	
	\item \textbf{Recuperación de la Solución:} Una vez completada la tabla de programación dinámica, la identificación del estado óptimo en $DP[n]$ requiere examinar todos los estados finales, operación que toma $O(k^\omega)$ tiempo. La reconstrucción de la coloración completa a partir del estado óptimo ya está disponible implícitamente en la información almacenada durante la construcción de la tabla, por lo que no añade complejidad asintótica adicional.
\end{itemize}

La complejidad total del algoritmo es $O(n^2 + n \cdot k^{\omega+1} \cdot \omega)$. Para grafos de intervalo con cliques pequeñas, específicamente cuando $\omega$ es constante o crece logarítmicamente con $n$, esta expresión es polinomial en $n$ y $k$. Sin embargo, para grafos de intervalo densos donde $\omega$ es proporcional a $n$, la complejidad se vuelve exponencial, reflejando la naturaleza inherentemente difícil del problema en tales instancias.

\paragraph{Demostración de Correctitud:}

Demostraremos que el algoritmo calcula el costo mínimo de coloración para un grafo de intervalo $G$ utilizando inducción sobre el número de vértices procesados.
	
	\textbf{Preposición:} Para cada $i \in \{1, 2, \dots, n\}$ y cada estado $\sigma$ en $DP[i]$, el valor $DP[i][\sigma]$ representa el costo mínimo de colorear los vértices $\{v_1, v_2, \dots, v_i\}$ de manera propia, sujeto a que los intervalos activos en la posición $i$ tengan exactamente las coloraciones especificadas por $\sigma$.
	
	\textbf{Base ($i=1$):} Para el primer vértice $v_1$, el algoritmo establece $DP[1][\{(v_1, c)\}] = C[v_1, c]$ para cada color $c$. Esta asignación es correcta porque $v_1$ es el único vértice procesado, no tiene restricciones de vecindad previas, y el costo de esta solución parcial es precisamente el costo de asignar el color $c$ al vértice $v_1$. Por tanto, la proposición se cumple para $i = 1$.
	
	\textbf{Paso Inductivo:} Supongamos que la proposición es cierta para $i-1$. Queremos demostrar que es cierta para $i$. Sea $\sigma$ un estado en $DP[i]$. Por construcción del algoritmo, $\sigma$ se obtiene al extender algún estado $\sigma'$ de $DP[i-1]$ mediante la asignación de un color $c$ al vértice $v_i$.
	
	Demostraremos primero la factibilidad. El estado $\sigma'$ representa una coloración propia de $\{v_1, \dots, v_{i-1}\}$ por la hipótesis de inducción. Al agregar $v_i$ con color $c$, el algoritmo verifica que $c$ sea diferente del color asignado a cada vecino de $v_i$ que está incluido en $\sigma'_{\text{activo}}$. Dado que $\sigma'_{\text{activo}}$ contiene precisamente los vértices cuyo intervalo se solapa con el intervalo de $v_i$, y dado que en un grafo de intervalo dos vértices son adyacentes si y solo si sus intervalos se solapan, la verificación de compatibilidad garantiza que la coloración extendida sigue siendo propia. Por tanto, $\sigma$ representa una coloración factible.
	
	Demostraremos ahora la optimalidad. El costo asociado a $\sigma$ es $DP[i][\sigma] = DP[i-1][\sigma'] + C[v_i, c]$, donde $\sigma'$ es el estado previo desde el cual se extendió. Por la hipótesis de inducción, $DP[i-1][\sigma']$ es el costo mínimo de colorear $\{v_1, \dots, v_{i-1}\}$ con las coloraciones especificadas en $\sigma'$. El costo total de extender esta solución agregando $v_i$ con color $c$ es precisamente la suma de estos dos términos. El algoritmo considera todas las combinaciones válidas de estados previos y colores para $v_i$, y retiene el costo mínimo para cada estado resultante $\sigma$. Por tanto, $DP[i][\sigma]$ es efectivamente el costo mínimo de alcanzar el estado $\sigma$ en la posición $i$.
	
Al llegar a la posición $n$, todos los vértices han sido procesados. El algoritmo identifica el estado $\sigma^*$ en $DP[n]$ con costo mínimo. Por el argumento inductivo, este estado representa una coloración propia completa del grafo con costo mínimo. Ninguna otra coloración propia puede tener menor costo, porque cualquier coloración propia debe pasar por algún estado en $DP[n]$, y el algoritmo ha considerado todos ellos. Por tanto, el algoritmo es correcto y encuentra la solución óptima.\\

\textbf{Limitación Práctica:} La limitación más fundamental es que el algoritmo solo es aplicable a grafos de intervalo genuinos, no a grafos cordales arbitrarios. Para grafos cordales que no admiten representación de intervalos, el algoritmo rechazará la instancia en la fase de preprocesamiento.

\textbf{Dependencia del Tamaño de la Clique Máxima:} El desempeño del algoritmo depende críticamente del tamaño de la clique máxima $\omega$ del grafo. Para grafos de intervalo con cliques pequeñas, el algoritmo es eficiente y práctico. Sin embargo, para grafos de intervalo densos donde $\omega$ se aproxima a $n$, la complejidad $k^{\omega+1}$ hace que el algoritmo sea impracticable incluso para valores moderados de $n$ y $k$. Esta sensibilidad a la densidad del grafo debe considerarse al seleccionar instancias apropiadas para este método.

\textbf{Construcción de la Representación de Intervalos:} La verificación de si un grafo cordal es efectivamente de intervalo y la construcción de una representación válida de intervalos son problemas no triviales. El algoritmo presentado utiliza una heurística basada en el ordenamiento de eliminación perfecta para construir esta representación.
...
\textbf{Descripción del Algoritmo:}Este procedimiento toma como entrada la solución factible $S_{greedy}$ generada por el algoritmo de aproximación anterior y busca iterativamente mejoras incrementales explorando el vecindario de la solución actual. El algoritmo sigue una estrategia de \textit{Hill Climbing} (escalada de colinas) con descenso estricto en el costo. 

Sea $\mathcal{C}: V \to \{1, \dots, k\}$ la coloración actual. En cada iteración, el algoritmo explora, para cada vértice $v \in V$, si existe un color alternativo $f' \neq \mathcal{C}(v)$ tal que:
\begin{enumerate}
	\item \textbf{Factibilidad:} El cambio de color es válido, es decir, ningún vecino de $v$ tiene el color $f'$:$$ \forall u \in N(v), \mathcal{C}(u) \neq f' $$
	\item \textbf{Mejora de Costo:} El costo de asignar el color $f'$ a $v$ es estrictamente menor al costo actual:$$ c_{v, f'} < c_{v, \mathcal{C}(v)} $$
\end{enumerate}

Si se encuentran múltiples colores que satisfacen estas condiciones para un vértice $v$, se selecciona aquel que minimiza el costo local (estrategia \textit{Best-Improvement} a nivel de vértice). Si el cambio se efectúa, la solución actual y el costo total se actualizan inmediatamente.\\

\textbf{Convergencia y Complejidad:} El algoritmo itera sobre todos los vértices del grafo repetidamente hasta que se cumple uno de los siguientes criterios de parada:
\begin{itemize}
	\item \textbf{Óptimo Local:} Se completa una pasada completa sobre todos los vértices sin realizar ningún cambio (la solución ha alcanzado un mínimo local).
	\item \textbf{Límite de Iteraciones:} Se alcanza un número máximo de pasadas predefinido (en la implementación, $MaxIter = 50$) para evitar ciclos o tiempos de ejecución excesivos en instancias muy grandes.
\end{itemize}
...
\textbf{Componentes del Algoritmo:}

\begin{enumerate}
	\item \textbf{Espacio de Estados:} $\mathcal{S}$ es el conjunto de todas las coloraciones propias válidas $\phi: V \to F$.
	
	\item \textbf{Función de Costo:} $C: \mathcal{S} \to \mathbb{R}^+$ donde $C(\phi) = \sum_{v \in V} c_{v,\phi(v)}$.
	
	\item \textbf{Vecindario:} Una coloración $\phi'$ es vecina de $\phi$ si difieren en exactamente un vértice y $\phi'$ es factible. Formalmente:
	\[
	\mathcal{N}(\phi)
	=
	\left\{
	\phi' \in \mathcal{S}
	\;:\;
	\left|\left\{ v \in V \;:\; \phi(v) \neq \phi'(v) \right\}\right| = 1
	\;\wedge\;
	\forall (u,w) \in E,\; \phi'(u) \neq \phi'(w)
	\right\}
	\]
	
	\item \textbf{Criterio de Aceptación (Metropolis):} El cambio de costo se define como $\Delta C = C(\phi') - C(\phi)$.
	\begin{itemize}
		\item Si $\Delta C \le 0$ (mejora o mantiene), el movimiento se acepta con probabilidad 1.
		\item Si $\Delta C > 0$ (empeoramiento), el movimiento se acepta con probabilidad:
		\[
		P_{\text{accept}} = e^{-\Delta C / T}
		\]
	\end{itemize}
	
	\item \textbf{Esquema de Enfriamiento:} La temperatura $T$ se reduce según:
	\[
...
\paragraph{Análisis de Correctitud}

\begin{proposition}[Terminación Determinística]
	\label{prop:sa-termination}
	El algoritmo Simulated Annealing con enfriamiento geométrico $T_{k+1} = \alpha \cdot T_k$, temperatura inicial $T_0 > 0$, temperatura mínima $T_{\min} > 0$, y límite máximo de iteraciones $\texttt{MAX\_ITER}$, termina en tiempo finito.
\end{proposition}

\begin{proof}
	El algoritmo termina cuando se cumple alguna de las siguientes condiciones:
	
	\textbf{Caso 1 (Temperatura mínima):} El número de niveles de temperatura $L$ hasta alcanzar $T_{\min}$ es:
	\[
	L = \left\lceil \frac{\log(T_{\min}/T_0)}{\log(\alpha)} \right\rceil
	\]
	
	Demostración: Después de $k$ iteraciones del bucle externo, la temperatura es $T_k = T_0 \cdot \alpha^k$. El bucle termina cuando:
	\[
	...
	\textbf{Caso 2 (Iteraciones máximas):} El bucle externo termina si $k \geq \texttt{MAX\_ITER}$, garantizando terminación por construcción.
	
	\textbf{Caso 3 (No mejora):} El algoritmo termina si no hay mejora en $\texttt{MAX\_NON\_IMPROVING}$ iteraciones consecutivas.
	
	Como al menos una de estas condiciones se cumple en tiempo finito, el algoritmo termina.
\end{proof}

\begin{lemma}[Preservación de Factibilidad con Vecindario Restringido]
	\label{lem:sa-feasibility}
	Sea $\phi_0 \in \mathcal{S}$ una coloración propia inicial. Si el vecindario $\mathcal{N}(\phi)$ solo contiene coloraciones propias, entonces para todo $k \geq 0$, la solución $\phi_k$ visitada en la iteración $k$ es una coloración propia.
\end{lemma}

\begin{proof}
	Procederemos por inducción sobre $k$.
	
	\textbf{Base ($k=0$):} Por hipótesis, $\phi_0 \in \mathcal{S}$ es una coloración propia.
	
	\textbf{Paso inductivo:} Supongamos que $\phi_k \in \mathcal{S}$ es una coloración propia. En la iteración $k+1$, el algoritmo genera un vecino $\phi' \in \mathcal{N}(\phi_k)$.
	...
	Por tanto, $\phi' \in \mathcal{S}$ es una coloración propia. El algoritmo acepta o rechaza $\phi'$ según el criterio, pero en cualquier caso:
	\[
	\phi_{k+1} \in \{\phi_k, \phi'\} \subseteq \mathcal{S}
	\]
	
	Por inducción, $\phi_k \in \mathcal{S}$ para todo $k \geq 0$.
\end{proof}

\begin{corollary}[Garantía de Salida Factible]
	Si la solución inicial $\phi_0$ es una coloración propia y el vecindario está restringido a $\mathcal{S}$, entonces el algoritmo SA retorna una coloración propia válida.
\end{corollary}

\begin{proof}
	Consecuencia directa de la Proposición \ref{prop:sa-termination} (el algoritmo termina) y el Lema \ref{lem:sa-feasibility} (todas las soluciones visitadas son factibles).
\end{proof}

\textbf{Nota sobre Convergencia Asintótica:} Existe un resultado teórico clásico de Geman y Geman (1984) que establece convergencia al óptimo global bajo enfriamiento logarítmico $T(k) \geq c/\log(1+k)$. Sin embargo, este esquema requiere tiempo exponencial y no es aplicable a implementaciones prácticas. El enfriamiento geométrico usado en la práctica ($T_{k+1} = \alpha \cdot T_k$ con $\alpha \in [0.8, 0.99]$) \textbf{no garantiza} convergencia al óptimo global en tiempo polinomial. La calidad de la solución depende empíricamente de:

\begin{itemize}
	\item La temperatura inicial $T_0$ y el factor de enfriamiento $\alpha$
	\item El número de iteraciones por temperatura $\texttt{MAX\_ITER}$
	\item La estructura del vecindario y la topología del grafo
	\item La distribución de la matriz de costos $C$
\end{itemize}


\paragraph{Análisis de Complejidad Temporal}

\begin{theorem}[Complejidad Temporal de SA]
	\label{thm:sa-complexity}
	Con un esquema de enfriamiento geométrico $T_{k+1} = \alpha \cdot T_k$, el algoritmo SA para MCCPP tiene complejidad temporal en el peor caso:
	\[
	T_{\text{SA}} = O\left(L \cdot \texttt{MAX\_ITER} \cdot (n + m)\right)
	\]
	donde $L = \left\lceil \frac{\log(T_{\min}/T_0)}{\log(\alpha)} \right\rceil$ es el número de niveles de temperatura, $n = |V|$ y $m = |E|$.
\end{theorem}

\begin{proof}
	Analizamos cada componente del algoritmo por separado:
	
	\textbf{Bucle externo (niveles de temperatura):}
	
	El número de iteraciones del bucle externo es exactamente $L$ (ver Proposición \ref{prop:sa-termination}). Con parámetros típicos ($T_0 = 100$, $T_{\min} = 0.01$, $\alpha = 0.95$):
	\[
	...
	Por cada nivel de temperatura, se ejecutan exactamente $\texttt{MAX\_ITER}$ movimientos. En implementaciones típicas, $\texttt{MAX\_ITER} = 100 \cdot n$ para asegurar suficiente exploración.
	...
	\begin{enumerate}
		\item \textbf{Generar vecino:} 
		\begin{itemize}
			\item Seleccionar vértice $v$ aleatorio: $O(1)$
			\item Seleccionar color $c$ aleatorio: $O(1)$
			\item Verificar factibilidad contra vecinos $N(v)$: $O(\deg(v))$
		\end{itemize}
		...
		\begin{itemize}
			\item Cálculo incremental: $\Delta C = c_{v,c_{\text{new}}} - c_{v,c_{\text{old}}}$ es $O(1)$
			\item Recalculo completo (si necesario): $O(n)$
		\end{itemize}
		
		\item \textbf{Criterio de aceptación:}
		\begin{itemize}
			\item Calcular probabilidad y generar número aleatorio: $O(1)$
		\end{itemize}
	\end{enumerate}
	...
	\[
	T_{\text{SA}} = L \cdot \texttt{MAX\_ITER} \cdot O(n + m) = O(L \cdot \texttt{MAX\_ITER} \cdot (n + m))
	\]
	
	Con $L = O(\log(T_0/T_{\min})/\log(1/\alpha))$ y $\texttt{MAX\_ITER} = O(n)$ típicamente, obtenemos:
	\[
	T_{\text{SA}} = O(n \cdot \log(T_0/T_{\min}) \cdot (n + m) / \log(1/\alpha))
	\]
	
	Para valores fijos de los parámetros de temperatura, esto se simplifica a $O(n(n+m))$. \qed
\end{proof}

\textbf{Optimizaciones Prácticas:}
En implementaciones eficientes, el costo por movimiento se puede reducir a $O(1)$ amortizado mediante:
\begin{itemize}
	\item Cálculo incremental de costos manteniendo $\sum_{v} c_{v,\phi(v)}$
	\item Verificación local de factibilidad solo en vecinos inmediatos
	\item Estructuras de datos auxiliares para colores disponibles
\end{itemize}

Con estas optimizaciones, la complejidad práctica es:
\[
T_{\text{SA}}^{\text{opt}} = O(L \cdot \texttt{MAX\_ITER}) = O(n \cdot \log(T_0/T_{\min}) / \log(1/\alpha))
\]

\textbf{Garantías Teóricas:} No existen garantías de factor de aproximación constante o logarítmico para SA con enfriamiento geométrico en grafos generales.

\textbf{Comportamiento Empírico:} En instancias de prueba del benchmark (ver Sección 4), SA con parámetros bien calibrados produce soluciones con calidad típica del 95-99\% respecto al óptimo conocido en instancias pequeñas, y competitivas con otras metaheurísticas en instancias grandes.

\textbf{Peores Casos Conocidos:} SA puede quedar atrapado en óptimos locales en:
\begin{itemize}
	\item Grafos con paisaje energético de "mesetas" (muchas soluciones con costo similar)
	\item Matrices de costo adversariales donde movimientos locales no correlacionan con mejora global
	\item Enfriamiento demasiado rápido ($\alpha > 0.95$) que causa convergencia prematura
\end{itemize}
...
\begin{enumerate}
	\item \textbf{Población Elite:} Conjunto $\mathcal{E}$ de a lo sumo $\ell$ soluciones de alta calidad:
	\[
	\mathcal{E} = \{(\phi_i, C(\phi_i)) : i \in [1,\ell], \phi_i \in \mathcal{S}, C(\phi_1) \leq \cdots \leq C(\phi_\ell)\}
	\]
	
	\item \textbf{Función Objetivo Penalizada:} Para manejar soluciones temporalmente infactibles:
	\[
	...
	\item \textbf{Búsqueda Local Tabú:} Explora el vecindario $\mathcal{N}(\phi)$ manteniendo una lista tabú $\mathcal{T}$ de movimientos recientes prohibidos (tamaño $\tau \approx 10$).
	
	\item \textbf{Path Relinking (PR):} Genera una trayectoria entre dos soluciones elite $\phi_{\text{inicio}}, \phi_{\text{fin}} \in \mathcal{E}$:
	\[
	\text{PR}(\phi_{\text{inicio}}, \phi_{\text{fin}}) = \{\phi^{(0)}, \phi^{(1)}, \ldots, \phi^{(d)}\}
	\]
	...
	\item \textbf{Diversificación:} Perturbación controlada aplicada cada $\delta$ iteraciones sin mejora para alejar la solución actual de un óptimo local, forzando la exploración de nuevas regiones del espacio. Una \textit{tabu list} de corta duración evita revertir inmediatamente la perturbación. Los reinicios se realizan si no hay mejora tras un número \(\kappa\) de iteraciones.
\end{enumerate}

\textbf{Estructura del Algoritmo:}

El algoritmo TSH+PR ejecuta un bucle principal de longitud $\texttt{MAX\_ITER}$ donde en cada iteración $t$ se realizan las siguientes operaciones:

\begin{itemize}
	\item Se selecciona una solución actual $\phi_t$ del conjunto elite $\mathcal{E}$
	\item Se aplica búsqueda local tabú sobre $\phi_t$ para obtener $\phi_t'$
	\item Si $C(\phi_t') < C(\phi_{\text{best}})$, se actualiza la mejor solución global
	\item Se actualiza el conjunto elite $\mathcal{E}$ con $\phi_t'$ si su calidad lo justifica
	\item Cada $\kappa$ iteraciones, se aplica Path Relinking entre dos soluciones de $\mathcal{E}$
	\item Cada $\delta$ iteraciones sin mejora, se aplica diversificación
	\item El algoritmo termina cuando $t = \texttt{MAX\_ITER}$ o si se detecta convergencia
\end{itemize}

\paragraph{Análisis de Correctitud}

\begin{theorem}[Terminación de TSH+PR]
	\label{thm:tshpr-termination}
	El algoritmo TSH+PR con parámetros $\texttt{MAX\_ITER}$, $\tau$ (tamaño lista tabú), $\kappa$ (frecuencia PR), y $\delta$ (frecuencia diversificación) termina en exactamente $\texttt{MAX\_ITER}$ iteraciones o antes.
\end{theorem}

\begin{proof}
	El algoritmo ejecuta un bucle principal controlado por un contador de iteraciones $t$ que se inicializa en $t = 0$ y se incrementa en cada iteración: $t \gets t + 1$. El bucle continúa mientras $t < \texttt{MAX\_ITER}$.
	
	Cada iteración ejecuta un número finito y acotado de operaciones:
	\begin{itemize}
		\item \textbf{Selección de solución actual:} Una operación de selección sobre $\mathcal{E}$ con $|\mathcal{E}| \leq \ell$ es finita.
		
		\item \textbf{Búsqueda local:} Explora a lo sumo $n \cdot k$ vecinos (número finito), donde cada evaluación requiere operaciones finitas de verificación de factibilidad y cálculo de costo.
		
		\item \textbf{Path Relinking:} Cuando se ejecuta (cada $\kappa$ iteraciones), genera a lo sumo $n$ soluciones intermedias, cada una requiriendo operaciones finitas.
		
		\item \textbf{Diversificación:} Cuando se ejecuta (cada $\delta$ iteraciones), modifica a lo sumo $0.25n$ vértices, operación finita.
		
		\item \textbf{Actualización de estructuras:} Inserción en lista tabú (tamaño acotado $\tau$), actualización de elite pool (tamaño acotado $\ell$), operaciones finitas.
	\end{itemize}
	
	Como cada iteración requiere tiempo finito y el contador $t$ se incrementa monótonamente hasta alcanzar $\texttt{MAX\_ITER}$, el algoritmo termina después de a lo sumo $\texttt{MAX\_ITER}$ iteraciones.
	
	\textbf{Terminación anticipada:} El algoritmo incluye una condición adicional de parada: si en las últimas 50 iteraciones no ha habido cambio en $C(\phi_{\text{best}})$, el bucle termina. Esta condición solo puede evaluarse después de al menos 50 iteraciones, y si se cumple, causa terminación inmediata. Por tanto, esta condición también garantiza terminación en tiempo finito (a lo sumo en iteración $t = 100$ si la convergencia es perfecta desde el inicio). \qed
\end{proof}

\begin{lemma}[Mantenimiento de Mejor Solución Factible]
	\label{lem:tshpr-feasibility}
	Sea $\phi_{\text{best}}$ la mejor solución encontrada por TSH+PR. Si la población inicial $\{\phi_1^{(0)}, \ldots, \phi_p^{(0)}\}$ contiene al menos una solución factible $\phi_i^{(0)} \in \mathcal{S}$, entonces al terminar el algoritmo, $\phi_{\text{best}} \in \mathcal{S}$ es una coloración propia.
\end{lemma}

\begin{proof}
	Analizamos la evolución de $\phi_{\text{best}}$ durante la ejecución del algoritmo mediante un argumento de invariante.
	
	\textbf{Estado inicial (iteración $t=0$):}
	...
	Para cada solución candidata $\phi$ de la población inicial, se verifica la condición:
	\[
	\text{is\_proper\_coloring}(G, \phi) \iff \forall (u,v) \in E, \phi(u) \neq \phi(v)
	\]
	...
	\textbf{Invariante de bucle:} Demostraremos que para toda iteración $t \geq 0$:
	\[
	I(t): \phi_{\text{best}}^{(t)} \in \mathcal{S} \land C(\phi_{\text{best}}^{(t)}) \leq C(\phi_{\text{best}}^{(t-1)})
	\]
	
	\textbf{Base ($t=0$):} Ya demostramos que $\phi_{\text{best}}^{(0)} \in \mathcal{S}$.
	
	\textbf{Paso inductivo:} Supongamos $I(t-1)$ cierto. En la iteración $t$, $\phi_{\text{best}}$ puede actualizarse en tres momentos:
	
	\textbf{Caso 1 - Tras búsqueda local:}
	
	La búsqueda local genera una solución mejorada $\phi_t'$ a partir de la solución actual $\phi_t \in \mathcal{E} \subseteq \mathcal{S}$. La actualización de $\phi_{\text{best}}$ ocurre únicamente si:
	\[
	\text{is\_proper\_coloring}(G, \phi_t') = \text{True} \land C(\phi_t') < C(\phi_{\text{best}}^{(t-1)})
	\]
	
	Esta condición se verifica explícitamente en el código mediante la función \texttt{is\_proper\_coloring}. Por tanto, si la actualización ocurre:
	\[
	\phi_{\text{best}}^{(t)} = \phi_t' \in \mathcal{S} \land C(\phi_{\text{best}}^{(t)}) < C(\phi_{\text{best}}^{(t-1)})
	\]
	
	Si la condición no se cumple, $\phi_{\text{best}}$ no se modifica y $I(t)$ se mantiene por $I(t-1)$.
	
	\textbf{Caso 2 - Tras Path Relinking:}
	
	El procedimiento Path Relinking genera una secuencia de soluciones intermedias $\{\phi^{(0)}, \phi^{(1)}, \ldots, \phi^{(d)}\}$ entre dos soluciones elite $\phi_{\text{inicio}}, \phi_{\text{fin}} \in \mathcal{E} \subseteq \mathcal{S}$.
	
	Para cada paso $i$ en la trayectoria, se modifica exactamente un vértice $v_i$:
	\[
	\phi^{(i)}[v] = \begin{cases}
		\phi_{\text{fin}}[v] & \text{si } v = v_i \\
		\phi^{(i-1)}[v] & \text{si } v \neq v_i
	\end{cases}
	\]
	...
	\[
	\text{is\_proper\_coloring}(G, \phi^{(i)}) = \text{True}
	\]
	
	Si la verificación falla, el movimiento se revierte: $\phi^{(i)} \gets \phi^{(i-1)}$, y se continúa con el siguiente vértice.
	
	La actualización de $\phi_{\text{best}}$ durante PR ocurre únicamente si:
	\[
	\exists i \in [0,d]: \text{is\_proper\_coloring}(G, \phi^{(i)}) \land C(\phi^{(i)}) < C(\phi_{\text{best}}^{(t-1)})
	\]
	
	Si tal $i$ existe, se actualiza:
	\[
	\phi_{\text{best}}^{(t)} = \phi^{(i)} \in \mathcal{S} \land C(\phi_{\text{best}}^{(t)}) < C(\phi_{\text{best}}^{(t-1)})
	\]
	
	Si ninguna solución en la trayectoria mejora $\phi_{\text{best}}$ o todas son infactibles, $\phi_{\text{best}}$ no se modifica y $I(t)$ se mantiene.
	
	\textbf{Caso 3 - Tras diversificación:}
	
	La diversificación genera una solución perturbada $\phi_{\text{div}}$ modificando aleatoriamente colores de vértices. Esta solución se evalúa y se actualiza el conjunto elite mediante el procedimiento:
	
	\texttt{\_update\_elite\_pool}$(\mathcal{E}, \phi_{\text{div}}, C(\phi_{\text{div}}), G, C, \ell)$
	
	Este procedimiento verifica explícitamente:
	\[
	\text{is\_proper\_coloring}(G, \phi_{\text{div}}) = \text{True}
	\]
	
	Si la verificación falla, $\phi_{\text{div}}$ se descarta inmediatamente y no se añade a $\mathcal{E}$. Solo si es factible y de calidad suficiente se añade a $\mathcal{E}$.
	
	Como $\phi_{\text{best}}$ solo se actualiza con soluciones de $\mathcal{E}$, y todas las soluciones en $\mathcal{E}$ son factibles, se mantiene $I(t)$.
	
	\textbf{Conclusión:} En todos los casos posibles de actualización, $\phi_{\text{best}}^{(t)} \in \mathcal{S}$ se mantiene. Por inducción, $I(t)$ es verdadero para todo $t \in [0, \texttt{MAX\_ITER}]$. Al terminar el algoritmo en iteración $T$, tenemos:
	\[
...
\textbf{Sobre Path Relinking e Infactibilidad Temporal:}
Es importante notar que Path Relinking \textbf{puede generar soluciones intermedias infactibles} durante la trayectoria entre $\phi_{\text{inicio}}$ y $\phi_{\text{fin}}$. Específicamente, si $\phi_{\text{inicio}}$ y $\phi_{\text{fin}}$ difieren en el color de un vértice $v$ que tiene vecinos, entonces al cambiar el color de $v$ de $\phi_{\text{inicio}}[v]$ a $\phi_{\text{fin}}[v]$ sin modificar otros vértices, puede crearse temporalmente un conflicto:
\[
...
\begin{enumerate}
	\item Se detectan inmediatamente mediante la verificación $\text{is\_proper\_coloring}(G, \phi^{(i)})$
	\item Se revierten antes de continuar: el algoritmo mantiene la solución factible anterior
	\item No se consideran para actualizar $\phi_{\text{best}}$
	\item Se evalúan con la función objetivo penalizada $f(\phi) = C(\phi) + M \cdot \text{conflictos}(\phi)$ únicamente para propósitos de guiar la búsqueda hacia regiones factibles
\end{enumerate}

Por tanto, aunque la exploración de PR puede visitar soluciones infactibles, la garantía del Lema \ref{lem:tshpr-feasibility} no se ve afectada, ya que ninguna solución infactible puede ser asignada a $\phi_{\text{best}}$.

\begin{corollary}[Garantía de Salida Válida]
	Si TSH+PR recibe una población inicial con al menos una solución factible, entonces el algoritmo retorna una coloración propia $\phi_{\text{best}} \in \mathcal{S}$ con costo finito $C(\phi_{\text{best}}) < \infty$.
\end{corollary}

\begin{proof}
	Consecuencia directa del Teorema \ref{thm:tshpr-termination} (el algoritmo termina en tiempo finito) y el Lema \ref{lem:tshpr-feasibility} ($\phi_{\text{best}}$ es siempre una coloración propia). Como $\phi_{\text{best}} \in \mathcal{S}$, su costo es:
	\[
	C(\phi_{\text{best}}) = \sum_{v \in V} c_{v,\phi_{\text{best}}(v)} < \infty
	\]
	ya que todos los costos $c_{v,f}$ son finitos por definición del problema.
\end{proof}

\paragraph{Análisis de Complejidad Temporal}

\begin{theorem}[Complejidad Temporal de TSH+PR]
	\label{thm:tshpr-complexity}
	La complejidad temporal del algoritmo TSH+PR para MCCPP es:
	\[
	T_{\text{TSH+PR}} = O(\texttt{MAX\_ITER} \cdot n \cdot k \cdot (n + m))
	\]
	donde $n = |V|$, $k = |F|$, $m = |E|$, y $\texttt{MAX\_ITER}$ es el número de iteraciones del bucle principal.
\end{theorem}

\begin{proof}
	Analizamos la complejidad de cada fase del algoritmo:
	
	\textbf{Fase 1: Construcción inicial (una sola vez)}
	\begin{itemize}
		\item Generar población inicial con heurísticas: $O(\text{population\_size} \cdot n \cdot m)$
		\item Seleccionar soluciones elite: $O(\text{population\_size} \cdot \log(\text{elite\_size}))$
		\item Total: $O(n \cdot m)$ (despreciable comparado con el bucle principal)
	\end{itemize}
	
	\textbf{Fase 2: Bucle principal ($\texttt{MAX\_ITER}$ iteraciones)}
	...
	\begin{enumerate}
		...
		\begin{itemize}
			\item Explorar vecindario de 1-intercambio: $n$ vértices $\times$ $k$ colores = $nk$ vecinos
			\item Por cada vecino:
			\begin{itemize}
				\item Calcular costo incremental: $O(1)$ 
				\item Verificar factibilidad (revisar vecinos): $O(\deg(v)) \leq O(m/n)$ en promedio
			\end{itemize}
			\item En implementación práctica, se limita a explorar $\min(10, n)$ vértices y $\min(10, k)$ colores
			\item Complejidad total: $O(nk \cdot \deg_{\text{avg}}) = O(nk \cdot m/n) = O(km)$
			\item Peor caso (sin limitación): $O(nk \cdot n) = O(n^2k)$
		\end{itemize}
		
		\item \textbf{Actualización de lista tabú:}
		\begin{itemize}
			\item Inserción en deque de tamaño acotado $\tau$: $O(1)$ amortizado
		\end{itemize}
		
		\item \textbf{Actualización de conjunto elite:}
		\begin{itemize}
			\item Verificar factibilidad: $O(m)$
			\item Insertar y ordenar (tamaño $\ell \approx 10$): $O(\ell \log \ell) = O(1)$
		\end{itemize}
		
		\item \textbf{Path Relinking (cada $\kappa$ iteraciones):}
		\begin{itemize}
			\item Frecuencia: $1/\kappa$ de las iteraciones (típicamente $\kappa = 50$)
			\item Distancia entre soluciones: $d = |\{v : \phi_1[v] \neq \phi_2[v]\}| \leq n$
			\item Por cada paso en la trayectoria (hay $d$ pasos):
			\begin{itemize}
				\item Modificar un vértice: $O(1)$
				\item Verificar factibilidad: $O(\deg(v)) \leq O(m/n)$
				\item Evaluar costo: $O(1)$ (incremental)
			\end{itemize}
			\item Complejidad total de un PR: $O(d \cdot m/n) = O(m)$
			\item Amortizado por iteración: $O(m/\kappa) = O(m/50)$
		\end{itemize}
		
		\item \textbf{Diversificación (cada $\delta$ iteraciones):}
		\begin{itemize}
			\item Frecuencia: $1/\delta$ de las iteraciones (típicamente $\delta = 50$)
			\item Perturbar $p = 0.15n$ vértices aleatorios: $O(p) = O(n)$
			\item Amortizado: $O(n/\delta) = O(n/50)$
		\end{itemize}
	\end{enumerate}
	...
	\textbf{Complejidad total:}
	\[
	T_{\text{TSH+PR}} = \texttt{MAX\_ITER} \cdot O(n^2k + m) = O(\texttt{MAX\_ITER} \cdot n \cdot k \cdot (n + m))
	\]
	
	\textbf{Análisis alternativo (verificación completa de factibilidad):}
	...
	\]
	\[
	T_{\text{TSH+PR}} = O(\texttt{MAX\_ITER} \cdot nkm)
	\]
	
	En la implementación práctica, se usa verificación local, por lo que la complejidad efectiva es la primera.
\end{proof}

\textbf{Optimizaciones de Implementación:}
	La complejidad práctica se reduce significativamente mediante:
	\begin{itemize}
		\item Limitar la exploración de vecindario a $\min(10, n)$ vértices
		\item Limitar colores probados a $\min(10, k)$
		\item Verificación local de factibilidad solo en vecinos inmediatos
		\item Cálculo incremental de costos
		\item Terminación anticipada en búsqueda local cuando se encuentra primera mejora
	\end{itemize}
	
	Con estas optimizaciones, la complejidad práctica es:
	\[
	T_{\text{TSH+PR}}^{\text{opt}} = O(\texttt{MAX\_ITER} \cdot \text{const} \cdot m)
	\]
	donde la constante es típicamente $\leq 100$.

\textbf{Garantías Teóricas:} TSH+PR no tiene un factor de aproximación probado analíticamente para grafos generales.

\textbf{Comportamiento Empírico:} Según el estudio de Araújo et al. \cite{araujo2020heuristicMCCPP}, TSH+PR alcanza soluciones con calidad del 97-99.5\% respecto al óptimo en instancias de benchmark, superando consistentemente a SA y otras metaheurísticas en instancias medianas y grandes ($n \geq 50$).

\textbf{Ventajas sobre SA:}
\begin{itemize}
	\item Exploración más dirigida mediante población elite
	\item Recombinación de características de buenas soluciones (PR)
	\item Menor sensibilidad a parámetros que SA
\end{itemize}

\textbf{Limitaciones:}
\begin{itemize}
	\item Mayor complejidad de implementación
	\item Requiere más memoria ($O(\ell \cdot n)$ para elite pool)
	\item Puede converger prematuramente si la diversidad de la población elite es baja
\end{itemize}
