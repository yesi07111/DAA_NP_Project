...
\begin{theorem}[Inaproximabilidad Fuerte del MCCPP]
	El Problema de Partición Cromática de Costo Mínimo (MCCPP) no pertenece a la clase APX. Específicamente, para cualquier constante $C > 1$, no existe un algoritmo de aproximación de factor $C$ en tiempo polinomial para el MCCPP, a menos que $P = NP$.
\end{theorem}

\begin{proof}
	La demostración procede mediante una reducción desde el problema del \textit{Número Cromático} (Graph Coloring), cuya inaproximabilidad es un resultado clásico. Sabemos que determinar el número cromático $\chi(G)$ de un grafo no admite aproximación de factor constante (de hecho, no admite aproximación $n^{1-\varepsilon}$ para cualquier $\varepsilon > 0$~\cite{hastad1999clique}).
	
	Supongamos, por contradicción, que existe un algoritmo $\mathcal{A}$ para el MCCPP con un factor de aproximación constante $\rho \ge 1$. Mostraremos que este algoritmo permitiría resolver el problema de \textit{Graph Coloring} con una precisión que viola los límites de inaproximabilidad conocidos.
	
	\paragraph{Construcción de la Instancia:}
	Sea $G = (V, E)$ una instancia arbitraria del problema de coloración con $n = |V|$ vértices. Construimos una instancia del MCCPP $\langle G, F, \mathcal{C} \rangle$ de la siguiente manera:
	\begin{itemize}
		\item El grafo $G$ se mantiene idéntico.
		\item El conjunto de frecuencias disponibles es $F = \{1, 2, \dots, n\}$.
		\item La matriz de costos se define utilizando \textit{pesos exponenciales}. Elegimos una base $W$ suficientemente grande, específicamente $W = \rho \cdot n + 1$. El costo de asignar el color $i$ a cualquier vértice $v$ es:
		\[ c_{v,i} = W^{i-1} \]
	\end{itemize}
	Esta construcción es polinomial, ya que la representación binaria de los pesos requiere $O(n \log W)$ bits, lo cual es polinomial en $n$.
	
	\paragraph{Análisis del Costo Óptimo:}
	Sea $\chi(G)$ el número cromático del grafo. La solución óptima del MCCPP utilizará exactamente los colores $\{1, \dots, \chi(G)\}$ para minimizar el costo (debido al crecimiento exponencial, siempre es preferible usar un color de índice menor).
	El costo óptimo $Z^*$ está acotado superiormente por el caso en que los $n$ vértices usen el color más costoso necesario, el color $\chi(G)$:
	\[ Z^* \le n \cdot W^{\chi(G)-1} \]
	
	\paragraph{Análisis de la Aproximación:}
	El algoritmo $\mathcal{A}$, al tener factor de aproximación $\rho$, produce una solución con costo $Z_{\mathcal{A}}$ tal que $Z_{\mathcal{A}} \le \rho \cdot Z^*$.
	Sea $k$ el índice del color más alto utilizado por la solución del algoritmo $\mathcal{A}$. Como al menos un vértice utiliza el color $k$, el costo de la solución está acotado inferiormente por el peso de ese solo vértice:
	\[ Z_{\mathcal{A}} \ge 1 \cdot W^{k-1} = W^{k-1} \]
	
	Combinando las desigualdades:
	\[ W^{k-1} \le Z_{\mathcal{A}} \le \rho \cdot Z^* \le \rho \cdot n \cdot W^{\chi(G)-1} \]
	
	Dividiendo ambos lados por $W^{\chi(G)-1}$:
	\[ W^{k - \chi(G)} \le \rho \cdot n \]
	
	Recordando que elegimos $W = \rho \cdot n + 1$, sustituimos $W$:
	\[ (\rho \cdot n + 1)^{k - \chi(G)} \le \rho \cdot n \]
	
	Analicemos el exponente $k - \chi(G)$. Dado que $k$ es el número de colores usado por una solución válida, necesariamente $k \ge \chi(G)$, por lo que el exponente es un entero no negativo.
	\begin{itemize}
		\item Si $k > \chi(G)$, entonces $k - \chi(G) \ge 1$. La desigualdad se convierte en:
		\[ \rho \cdot n + 1 \le \rho \cdot n \]
		Lo cual es una contradicción ($1 \le 0$).
		\item Por lo tanto, la única posibilidad lógica es que $k - \chi(G) = 0$, es decir, $k = \chi(G)$.
	\end{itemize}
	
	\paragraph{Conclusión:}
	Esto demuestra que si existiera un algoritmo de aproximación de factor constante $\rho$ para el MCCPP con esta estructura de costos, dicho algoritmo encontraría \textbf{exactamente} el número cromático $\chi(G)$ para cualquier grafo.
	Dado que calcular $\chi(G)$ es NP-difícil y no aproximable dentro de factores polinomiales, concluimos que no puede existir tal algoritmo $\mathcal{A}$.
	
	Por tanto, el MCCPP no pertenece a APX.
\end{proof}
