...
Este algoritmo utiliza un esquema de programación dinámica sobre un Ordenamiento de Eliminación Perfecta (PEO). A diferencia de la DP clásica en árboles, aquí el estado se extiende para garantizar la factibilidad contra todos los vecinos precedentes, aprovechando que la estructura de intervalos mantiene el conflicto acotado.
...
Nota de implementación: Cada estado $DP[i][c]$ mantiene un registro (diccionario) de la coloración parcial $\varphi_{i,c}$ que generó dicho costo mínimo para permitir verificaciones de consistencia en pasos futuros.

\textbf{R (Recurrencia):} Para $i > 0$:$$DP[i][c] = c_{v_i, c} + \min DP[i-1][c']$$

Sujeto a que la coloración asociada a $DP[i-1][c']$ sea compatible con la asignación del color $c$ al vértice $v_i$.
La condición de compatibilidad se define como:

$$\forall u \in \text{Adj}(v_i) \cap \{v_0, \dots, v_{i-1}\}, \quad \varphi_{i-1,c'}(u) \neq c$$

\textbf{T (Topología):} Orden lineal ascendente según el PEO ($i = 1$ hasta $n$).

\textbf{B (Base):} Para el primer vértice del ordenamiento: $DP[0][c] = c_{v_0, c}$ y $\varphi_{0,c} = \{v_0: c\} \quad \forall c \in F$.

\textbf{O (Objetivo):} $$Z^* = \min_{c \in F} DP[n-1][c]$$
...
La complejidad del algoritmo de Programación Dinámica para el MCCPP en grafos de intervalo, considerando la verificación exhaustiva de consistencia, es $O(n^2 \cdot k^2)$.

\begin{itemize}
	\item \textbf{Preprocesamiento:} Verificación de cordalidad y ordenamiento de eliminación perfecta: \(O(n + m)\)
	\item \textbf{Cálculo DP:} Existen $n$ etapas (una por cada vértice en el PEO). En cada etapa, se evalúan $k$ colores para el vértice actual $v_i$. Para cada color, se intenta extender la solución desde los $k$ estados posibles del nodo anterior $v_{i-1}$.
	\item \textbf{Verificación de Consistencia:} Para validar la transición, se comprueban las aristas entre $v_i$ y todos los vértices previos $v_j$ ($j < i$). Este proceso es $O(n)$ en el peor caso.
	\item \textbf{Complejidad total:} $n \cdot k \cdot k \cdot n = O(n^2 \cdot k^2)$.
\end{itemize}

NOTA: Aunque en la teoría de grafos de intervalo la verificación podría optimizarse a $O(\omega)$ (donde $\omega$ es el tamaño de la clique máxima) limitando la búsqueda a intervalos activos, la implementación actual asegura correctitud general sobre el PEO mediante una revisión histórica completa, resultando en la cota $O(n^2 \cdot k^2)$.
...
El algoritmo calcula el costo mínimo de la partición cromática para un grafo de intervalo $G$ siguiendo un PEO. Procederemos por inducción sobre el número de vértices procesados $i$.
	...
	\textbf{Paso Inductivo:} Supongamos que para $i-1$, $DP[i-1][c']$ contiene el costo mínimo de una coloración propia del subgrafo inducido por los primeros $i-1$ vértices donde $v_{i-1}$ tiene el color $c'$. Queremos probar que $DP[i][c]$ es factible y óptimo para $i$ vértices.
	
	\begin{enumerate}
		\item \textbf{Factibilidad:} Sea $v_i$ el vértice actual al que se le asigna el color $c$. La implementación verifica explícitamente que $\forall v_j \in \text{Adj}(v_i)$ con $j < i$, se cumpla $\varphi_{i-1, c'}(v_j) \neq c$. Por la propiedad del PEO, los vecinos precedentes de $v_i$ forman una clique; al verificar el historial completo almacenado en el estado de la DP, el algoritmo garantiza que la extensión de la coloración sigue siendo propia. Solo las transiciones que cumplen esta restricción de vecindad actualizan la tabla.
		
		\item \textbf{Optimalidad:} El costo total de colorear hasta $v_i$ es la suma del costo individual $C[v_i, c]$ y el costo de colorear el resto del grafo anterior. Dado que ya sabemos que los valores del paso $i-1$ son los mínimos posibles (por nuestra hipótesis) y que el algoritmo elige el menor de todos los resultados válidos tras probar todas las combinaciones de colores, el nuevo valor resultante en $DP[i][c]$ es necesariamente el costo mínimo para el prefijo de $i$ nodos.
	\end{enumerate}
	
Al llegar al último vértice ($i=n$), el algoritmo simplemente selecciona el valor más bajo de la última fila, lo que garantiza que hemos encontrado la solución óptima para el grafo completo.

\textbf{Limitación Práctica:} Este algoritmo requiere conocer o poder computar un ordenamiento de eliminación perfecta válido. Para grafos que no son cordales, el algoritmo no es aplicable directamente.
